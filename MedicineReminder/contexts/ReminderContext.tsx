...
export type ReminderContextType = {
    ...
    updateDeleteReminder: (reminder: Reminder, type: "delete" | "update") => Promise<void>
...
export const ReminderContextProvider = ({ children }: ReminderContextProviderProps) => {
    ...
    const updateDeleteReminder = async (reminder: Reminder, type: "delete" | "update") => {
        const newReminders = Array.from(reminders)
        const index = newReminders.findIndex((searchReminder) => searchReminder.id === reminder.id)
        if (index < 0) {
